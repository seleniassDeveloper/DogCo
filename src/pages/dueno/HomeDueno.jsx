...
export const HomeDueno = () => {
    ...
    const futuras = reservas
      .filter(
        (r) =>
          ...
          ["Confirmado", "Pendiente", "Activo"].includes(r.estado)
      )
  ...
  const kpis = useMemo(() => {
    const proximaEnDias = proxima
      ? Math.max(
          0,
          Math.ceil((new Date(proxima.start) - now) / (1000 * 60 * 60 * 24))
        )
      : "—";
    return {
      proximaEnDias,
  ...
  // ====== Actions ======
  const onVerDetalles = (r) =>
    alert(`Detalles de la reserva #${r.id} con ${r.cuidador}`);
  const onReprogramar = (r) => alert(`Reprogramar reserva #${r.id}`);
  const onCancelar = (r) =>
    window.confirm("¿Seguro que deseas cancelar esta reserva?") &&
    alert(`Reserva #${r.id} cancelada`);
  ...
  const onSubmitRequest = (req) => {
    ...
    setRequests((prev) => [
      { ...req, mascotaNombre: mascota?.nombre },
      ...prev,
    ]);
  };
  const onCancelRequest = (r) => {
    if (window.confirm("¿Cancelar esta solicitud?")) {
      setRequests((prev) =>
        prev.map((x) => (x.id === r.id ? { ...x, estado: "Cancelada" } : x))
      );
    }
  };

  const abrirRate = () => setShowRate(true);
  const abrirChat = (thread) => {
    setChatThread(thread);
    setChatOpen(true);
  };
  const onChat = (reserva) => {
    const th =
      mensajesThreads.find((t) => t.nombre === reserva.cuidador) ||
      mensajesThreads[0];
    abrirChat(th);
  };
  ...
  return (
    <div ref={rootRef} className="home-dueno-container">
      ...
      <div className="d-flex addboton justify-content-end pe-5 pt-3">
        ...
        <button
          className=" btn mx-3 botonListapaseo"
          onClick={() => handleNavigate()}
        >
          <i class="fa-solid fa-magnifying-glass"></i>Buscar Paseador
        </button>
      </div>
      <div className="px-5 mt-3">
        <KPICards kpis={kpis} />
      ...
      <div className="px-5 home-grid mt-3">
        ...
        <div className="colB">
          ...
          />
          <MessagesPreview
            threads={mensajesThreads}
            proxima={proxima}
            onChat={onChat}
            openThread={abrirChat}
          />
